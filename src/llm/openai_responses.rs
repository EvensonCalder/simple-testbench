...
pub fn execute_openai_responses_prompt(
    ...
    verbose: bool,
    request_label: &str,
) -> anyhow::Result<ResponsesExecution> {
    if model.api_style != ApiStyle::OpenaiResponses {
        return Err(anyhow!(
            "model {} is configured as {}, not openai_responses",
            model.model_id,
    ...
    let endpoint = provider
        .endpoints
        .endpoint_for(&ApiStyle::OpenaiResponses)
        .ok_or_else(|| {
            anyhow!(
                "provider {} does not support openai_responses",
                provider.provider_id
            )
        })?;
    let url = responses_url(endpoint);
    let api_key = resolve_api_key(provider)?;
    let request_body = build_responses_request(model, system_prompt_text, user_prompt);
        ...
        match super::streaming::block_on_http(send_request(
            ...
            Err(error) => {
                return Err(error.context(format!(
                    "responses request failed for {}/{} {} after {} attempts",
                    provider.provider_id, model.model_id, request_label, attempt_number
                )));
            }
        }
...
fn extract_output_text(response_text: &str) -> anyhow::Result<String> {
    let payload = serde_json::from_str::<Value>(response_text)
        .context("failed to parse responses response JSON")?;
    ...
    if parts.is_empty() {
        return Err(anyhow!(
            "response did not contain any final assistant text in output_text or output content"
        ));
    }
...
async fn extract_streaming_output_text(
    ...
    super::streaming::read_sse_response(response, idle_timeout, |data| {
        ...
        let payload = serde_json::from_str::<Value>(data)
            .context("failed to parse responses stream event JSON")?;
    ...
    if let Some(response) = completed_response {
        return extract_output_text(&response.to_string());
    }

    Err(anyhow!(
        "stream response did not contain response.output_text.delta"
    ))
}
...
mod tests {
    ...
    use super::{build_responses_request, extract_output_text};

    fn model() -> ModelConfig {
        ModelConfig {
            provider_id: "mock".to_string(),
            model_id: "responses-model".to_string(),
    ...
    #[test]
    fn builds_expected_responses_request() {
        let request = build_responses_request(&model(), "Return JSON", "buy milk");

        assert_eq!(request["model"], json!("responses-model"));
        assert_eq!(request["stream"], json!(true));
        assert_eq!(request["instructions"], json!("Return JSON"));
        assert_eq!(request["temperature"], json!(0.0));
        assert_eq!(request["max_output_tokens"], json!(256));
        assert_eq!(request["input"][0]["content"][0]["text"], json!("buy milk"));
    }

    #[test]
    fn extracts_output_text_and_ignores_reasoning_items() {
        let output = extract_output_text(
            r#"{"output":[{"type":"reasoning","summary":"hidden"},{"type":"message","content":[{"type":"output_text","text":"final answer"}]}]}"#,
        )
